...
const BalanceIsALie = () => {
  ...
  useEffect(() => {
    ...
    // --- AUDIO SYSTEM START ---
    const audio = {
      bgm: new Audio('/audio/game_bgm.mp3'),
      death: new Audio('/audio/death_bgm.mp3'),
      nextLevel: new Audio('/audio/next_level_bgm.mp3')
    };
    
    // Configure loops
    audio.bgm.loop = true;
    audio.nextLevel.loop = true;
    audio.death.loop = false;
    
    // Global volume adjustment if needed
    audio.bgm.volume = 0.5;
    audio.death.volume = 0.6;
    audio.nextLevel.volume = 0.6;

    function playTrack(track: 'bgm' | 'death' | 'nextLevel' | 'none') {
      // "Only ONE audio track may play at any time"
      // "When any other audio starts: Immediately pause or stop game_bgm"
      
      if (track !== 'bgm') {
        audio.bgm.pause();
        // We don't reset BGM time so it can resume
      }
      ...
      if (track !== 'nextLevel') {
        audio.nextLevel.pause();
        audio.nextLevel.currentTime = 0;
      }
      
      if (track !== 'none') {
        audio[track].play().catch(e => console.log("Audio play prevented:", e));
      }
    }
    // --- AUDIO SYSTEM END ---
    ...
    function killPlayer() {
      ...
      // Audio: Stop/pause bgm, Play death_bgm_2.0 once
      playTrack('death');
      
      // "Start a 3-second delay timer"
      // "After exactly 3 seconds... Respawn... Resume looping game_bgm"
      setTimeout(() => {
    ...
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (game.scene === 'playing' && (e.key === 'Escape' || e.key === 'Tab')) {
        ...
        playTrack('none'); // Pause audio when paused
        return;
      }
      ...
      if (game.scene === 'paused') {
        ...
        if (e.key === 'Enter' || e.key === ' ') {
          if (game.pauseSelection === 0) {
             ...
             playTrack('bgm'); // Keep playing in menu
          }
        }
        if (e.key === 'Escape') {
            game.scene = 'playing';
      ...
      if ((game.scene === 'credits' || game.scene === 'howToPlay' || game.scene === 'about' || game.scene === 'ending') && 
        ...
        if (game.scene === 'ending') {
            playTrack('bgm'); // Restart BGM after ending
        }
        return;
      }
      ...
      if (game.scene === 'levelComplete' && (e.key === 'Enter' || e.key === ' ')) {
        ...
        // Stop next_level_bgm, Resume looping game_bgm
        playTrack('bgm');
    ...
    function update() {
      ...
      if (game.levelState.complete) {
        ...
        if (game.levelState.doorClosing >= 60) {
            ...
            // Play next_level_bgm_2.0 in a loop
            playTrack('nextLevel');
        }
        return;
      }
